...
**** Description
     Highest priority subparser. Parse comments is run before all other parsers
     and is responsible for skipping across all comments it encounters.
**** Pseudo Code
     if current token is "/*" then
       advance current token variable until current token is "*/"
       if no more tokens and current token is not "*/"
